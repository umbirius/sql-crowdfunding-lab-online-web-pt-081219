...
def selects_the_titles_and_amount_over_goal_of_all_projects_that_have_met_their_funding_goal
  "SELECT projects.title, (SUM(pledges.amount GROUP BY pledges.project_id) - projects.funding_goal) FROM projects JOIN pledges ON projects.id = pledges.project_id WHERE (pledges.amount - projects.funding_goal) >= 0 GROUP BY pledges.project_id" 
end
